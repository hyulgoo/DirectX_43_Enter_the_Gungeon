...
void CPlayerScript::tick()
{
	...
	if (KEY_TAP(KEY::Q) && m_iBlankBullet > 0)
	{
		--m_iBlankBullet;
		CUIMgr::GetInst()->SetBlank(m_iBlankBullet);
		CGameObject* pBlank = new CGameObject;
		pBlank->AddComponent(new CTransform);
		pBlank->AddComponent(new CCollider2D);
		pBlank->AddComponent(new CMeshRender);
		pBlank->AddComponent(new CBlankScript);
		
		pBlank->MeshRender()->SetMesh(CResMgr::GetInst()->FindRes<CMesh>(L"RectMesh"));
		pBlank->MeshRender()->SetMaterial(CResMgr::GetInst()->FindRes<CMaterial>(L"DistortionMtrl"));
		SpawnGameObject(pBlank, Transform()->GetWorldPos(), (int)LAYER::BLANK);
		CResMgr::GetInst()->FindRes<CSound>(L"BulletKingShot")->Play(1, 1.f, true);
	}
