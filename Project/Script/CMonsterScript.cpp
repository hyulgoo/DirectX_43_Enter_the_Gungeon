...
void CMonsterScript::CheckDir()
{
	...
	m_vOwnerPos = Transform()->GetWorldPos();
	m_vPlayerPos = m_pPlayer->Transform()->GetWorldPos();

	m_fAngle = GetDirfloat(m_vOwnerPos, m_vPlayerPos, true);
	Vec2 vMontoPly = Vec2(m_vPlayerPos.x - m_vOwnerPos.x, m_vPlayerPos.y - m_vOwnerPos.y);
	m_fLengthtoPly = vMontoPly.Length();
	m_vDirToPly = Vec2(m_vPlayerPos.x - m_vOwnerPos.x, m_vPlayerPos.y - m_vOwnerPos.y);
