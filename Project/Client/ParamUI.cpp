...
int ParamUI::Param_Int(const string& _strDesc, int* _pData, bool _bDrag)
{
	...
	string strIntName = GetNextName("##Parma_Int");
...
int ParamUI::Param_Float(const string& _strDesc, float* _pData, bool _bDrag)
{
	...
	string strFloatName = GetNextName("##Parma_Float");
	ImGui::SetNextItemWidth(150.f);
...
int ParamUI::Param_Vec2(const string& _strDesc, Vec2* _pData, bool _bDrag)
{
	...
	string strVec2Name = GetNextName("##Parma_Vec2");
	ImGui::SetNextItemWidth(150.f);

	float arrFloat[2] = { _pData->x, _pData->y };
	if (_bDrag)
	{
		if (ImGui::DragFloat2(strVec2Name.c_str(), arrFloat))
		{
			_pData->x = arrFloat[0];
			_pData->y = arrFloat[1];
			return 1;
	...
	{
		if (ImGui::InputFloat2(strVec2Name.c_str(), arrFloat))
		{
			_pData->x = arrFloat[0];
			_pData->y = arrFloat[1];
			return 1;
...
int ParamUI::Param_Vec4(const string& _strDesc, Vec4* _pData, bool _bDrag)
{
	...
	string strVec4Name = GetNextName("##Parma_Vec4");
	ImGui::SetNextItemWidth(200.f);
	if (_bDrag)
	{
		if (ImGui::DragFloat4(strVec4Name.c_str(), *_pData))
			return 1;
	}
	else
	{
		if (ImGui::InputFloat4(strVec4Name.c_str(), *_pData))
			return 1;
	}
	return 0;
}
...
// 이름 설정을 정확히 하기 위해서 id 초기화를 반드시 해줄 것
string ParamUI::GetNextName(const string& _ShowName)
{
	// Text 등의 ID가 중복되는 것을 막기 위해 중복되지 않는 이름을 조합하여 만들어줌.
	char szKey[255] = {};
