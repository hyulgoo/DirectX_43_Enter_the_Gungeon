...
void CMesh::Create(void* _VtxSysMem, UINT _iVtxCount, void* _IdxSysMem, UINT _iIdxCount)
{
	m_VtxCount = _iVtxCount;
	m_IdxCount = _iIdxCount;
	// 시스템메모리에서 VtxCount만큼의 데이터를 알맞은 단위로 읽어옴
	m_pVtxSys = new Vtx[m_VtxCount];
	memcpy(m_pVtxSys, _VtxSysMem, sizeof(Vtx) * m_VtxCount);

	// 시스템메모리에서 IdxCount만큼의 데이터를 알맞은 단위로 읽어옴
