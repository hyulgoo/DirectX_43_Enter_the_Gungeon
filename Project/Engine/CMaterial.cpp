...
{
	// C++에서는  return 타입을 여러 가지로 지정할 수 없기 때문에
	// 입력받을 인잘를 받고 인자 타입에 따라 다르게 반환할 수 있도록 한다.
	// param의 타입에 맞는 param을 구하고 param에 타입을 Data에 안에 있는 데이터를 넣어준다.
	switch (_param)
	{
	case INT_0:
	case INT_1:
...
int CMaterial::Load(const wstring& _strFilePath)
{
	...
	// Texture
	for (UINT i = 0; (UINT)TEX_PARAM::TEX_END; ++i)
	{
