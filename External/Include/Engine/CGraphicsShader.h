...
private:
    ...
    vector<tScalarParam>          m_vecScalerParam;
    vector<tTexParam>             m_vecTexParam;


public:
    void CreateVertexShader(const wstring& _strFileNamem, const string& _strFuncName);
    ...
    void AddScalarParam(SCALAR_PARAM _Param, const string& _desc) { m_vecScalerParam.push_back(tScalarParam{ _Param, _desc }); }
    void AddTexParam(TEX_PARAM _Param, const string& _desc) { m_vecTexParam.push_back(tTexParam{ _Param, _desc }); }

    const vector<tScalarParam>& GetScalarParam() { return m_vecScalerParam; }
